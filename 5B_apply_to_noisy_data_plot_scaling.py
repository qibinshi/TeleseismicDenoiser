...
def line_fit(x, y, da=0.01, db=0.1):
    ...
    for a in np.arange(0.0, 2.0, da, dtype=np.float64):
        for b in np.arange(6, 40, db, dtype=np.float64):
            line_func = x * a - b
...
fig_dir = model_dir + '/apply_minSNR2_fixWin60'
...
ind_select = np.where(np.logical_and(nbin > 5, nsta > minsta))[0]
...
# %% Energy
ax[3, 0].scatter(moment, Es_noisy * moment * moment/(vp**6), marker='o', s=200, c=depth, edgecolors='k', cmap=cmap, vmin=v1, vmax=v2)
ax[3, 1].scatter(moment, Es_deno * moment * moment/(vp**6), marker='o', s=200, c=depth, edgecolors='k', cmap=cmap, vmin=v1, vmax=v2)
ax[3, 2].scatter(depth, Es_noisy * moment * moment/(vp**6), marker='o', s=200, c=mw, edgecolors='k', cmap=cmap1, vmin=v3, vmax=v4)
ax[3, 3].scatter(depth, Es_deno * moment * moment/(vp**6), marker='o', s=200, c=mw, edgecolors='k', cmap=cmap1, vmin=v3, vmax=v4)
ax[4, 0].scatter(moment, Es_noisy * moment* 30000/(vp**6), marker='o', s=200, c=depth, edgecolors='k', cmap=cmap, vmin=v1, vmax=v2)
ax[4, 1].scatter(moment, Es_deno * moment* 30000/(vp**6), marker='o', s=200, c=depth, edgecolors='k', cmap=cmap, vmin=v1, vmax=v2)
ax[4, 2].scatter(depth, Es_noisy * moment* 30000/(vp**6), marker='o', s=200, c=mw, edgecolors='k', cmap=cmap1, vmin=v3, vmax=v4)
ax[4, 3].scatter(depth, Es_deno * moment* 30000/(vp**6), marker='o', s=200, c=mw, edgecolors='k', cmap=cmap1, vmin=v3, vmax=v4)
...
for k in range(2):
    ...
    a, b = line_fit(np.log10(moment), np.log10(Es_deno * moment * moment/(vp**6)))
    print('E-M', a, b)
    lg_E = lg_mg * a - b
    ax[3, k].plot(10 ** lg_mg, 10 ** lg_E, '-', color='g', linewidth=5, alpha=1.0, label=r'$E_R$~$M_0^{1.58}$')
    ax[3, k].legend(loc='lower right')

    lg_E = lg_mg * 0.17 - 5.7 +4.5-6
    ax[4, k].plot(10 ** lg_mg, 10 ** lg_E, '-k', linewidth=5, alpha=1, label=r'$\sigma_A$~$M_0^{0.17}}$')
    lg_E = lg_mg * (a-1) - b + 4.5-6
    ax[4, k].plot(10 ** lg_mg, 10 ** lg_E, '-', color='g', linewidth=5, alpha=1, label=r'$\sigma_A$~$M_0^{0.58}$')
...
for i in range(4):
    ...
    ax[3, i].set_ylim(1e0, 5e18)
    ax[4, i].set_ylim(1e-10, 1e-1)
    ax[5, i].set_ylim(0, 3)
