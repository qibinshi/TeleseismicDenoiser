...
allwv = np.zeros((0, 600, 3), dtype=np.double)
onewv = np.zeros((1, 600, 3), dtype=np.double)
...
for ev in cat:
    ...
    for sta in  glob.glob(workdir + evnm + 'stas/*xml'):
        ...
        ## %% Read, remove response, filter, resample, trim and rotate
        try:
