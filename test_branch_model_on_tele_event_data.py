...
batch_size = 256
working_dir = './'
wave_mat = working_dir + 'wave_double.mat'
...
# %% Read mat data
wv = loadmat(wave_mat)["allwv"]
waveform_data = WaveformDataset(wv, wv)

# %% Model
model = torch.load(model_dir + '/' + f'{model_name}_Model.pth', map_location=devc)
test_iter = DataLoader(waveform_data, batch_size=batch_size, shuffle=True)
model.eval()
...
for i_model in range(10):
    ...
    plt.figure(1)
    plt.savefig(f'figures/{model_name}_Prediction_waveform_model_{i_model}.pdf', bbox_inches='tight')
...
#    ax[1, 0].set_ylabel('velocity spectra', fontsize=14)
    plt.figure(1)
    plt.savefig(f'figures/{model_name}_spectrum_{i_model}.pdf', bbox_inches='tight')
